...
class ReplyModel(Query):
    ...
    def get_all_replies_by_topic_id(self, topic_id, num = 15, current_page = 1):
        ...
        field = "reply.*, \
                ...
                user.fvteam as author_fvteam, \
                user.signature as author_signature"
        return self.where(where).order(order).join(join).field(field).pages(current_page = current_page, list_rows = num)

    def get_topics_all_replies_count(self, topic_id):
        where = "topic_id = %s" % topic_id
        join = "LEFT JOIN user ON reply.author_id = user.uid"
    ...
    def get_user_all_replies(self, uid, num = 15, current_page = 1):
